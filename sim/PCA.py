...
#directories=['{}/replica_tight_spacing'.format(protein)]
#temperatures=[['*.***']]

directories=['{}/replica_tight_spacing'.format(protein), '{}/unfolding'.format(protein), '{}/unfolding2'.format(protein)]   #directories containing results
temperatures=[['*.***'], ['*.***'], ['*.***']]
...
def Compute_rmsd(data, ref_file):
	"""
	Computes rmsd of all PDB snapshots, stored in data (should not be centered) with respect to ref file
	Returns a vertical array corresponding to rmsd values for each snapshot
	"""
	with warnings.catch_warnings(): #suppress all warnings
		warnings.simplefilter("ignore")
		reference=Bio.PDB.PDBParser().get_structure(protein,ref_file).get_residues()
	...
	ref_coords=np.reshape(ref_coords, (1,np.shape(ref_coords)[0]*3))	
	
	sd=np.multiply(data-ref_coords, data-ref_coords )
	rmsd=np.sqrt(np.mean(sd, axis=1))
	return np.reshape(rmsd, (len(rmsd), 1))
...
rmsd=Compute_rmsd(combined_Data+combined_mean, ref_file)

projections=np.hstack((rmsd, projections))
