...
void SetProgramOptions(int argc, char *argv[]) {
  ...
  /*A note about sscanf for a C beginner (namely Amir):
  sscanf reads formatted input, and expects you to tell it what address you should send that input to
  Thus, in general, you pass pointers after your formatted string (ex. &value below)
  This means that the float found in the string is sent to the address of the (float) variable value
  
  So then why don't we put an ampersand in front of token?
  The reason is that token was initiated as an ARRAY
  It turns out, the way C works, is that when you call an array by itself (ex. token), you are actually calling the POITNER for the first element of the array
  For instance, in sscanf below, when you call token, you are calling the pointer for the first element of token, exactly as sscanf wants you to do
  But that being said, if you call an element of an array, then you are calling the value
  
  For instance, token[i] is equivalent to *(token+i), where again, token is the ADDRESS for the array's first element, and the +i tells you to move along i elements in memory, so you are accessign the ith element of that array
  */
  while (fgets(line, 500, DATA) != NULL) {
    /* printf("%s", line);*/
    if (strncmp(line, "!", 1)) { /*Makes sure the line doesn't have an exclamation point in the first character, since this is a comment */
      sscanf(line, "%s %f", token, &value);  /*Each line is read according to the expected format "%s %f". In some cases, the second value will be string rather than a float, in whcih case I think value is left unassigned?*/
      if (!strcmp(token, "NATIVE_FILE")) {
	sscanf(line, "%*s %s", name);  /*This is one of those cases where the second value is a string, so instead of doing anything with the variable value, we just re-read the line as two strings*/
	...
	PRINT_PDB = (int) value;   /*A syntax note (for beginner): If you only have one line after the if clause, then you don't need curly brackets*/
	...
	MC_TEMP_MIN =  value;   /*I noticed that when the variable is an int, you assign it as (int) value, whereas for Float, you just assign it as value. */
  ...
  if (nprocs % NODES_PER_TEMP !=0){
    printf("ERROR! Number of nodes must be a multiple of NODES_PER_TEMP");
    exit(1);
  }
